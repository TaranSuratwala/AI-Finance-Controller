...
```mermaid
graph TD
    A[Razorpay Webhook] -->|HMAC Signed POST| B(FastAPI Server)
    B -->|200 OK| A
    B -.->|Background Task| C{Idempotency Check <br/> Redis}
    C -->|New| D[AI Agent <br/> Gemini 3.6 Flash]
    D -->|Extract & Propose| E{Gatekeeper <br/> Rules Engine}
    E -->|Pass| F[(Master Ledger)]
    E -->|Fail| G[(Audit SQLite DB)]
    G --> H[Streamlit <br/> FinOps Dashboard]
```
...
- **Duplicate webhook protection** via Redis idempotency handling.
